...
class MainPluginDialog(wx.Dialog):
    ...
    def __init__(self, board):
        ...
        wx.Dialog.__init__ ( self, None, id = wx.ID_ANY, title = "Output Fab v(%s)"%output_fab.__version__, pos = wx.DefaultPosition, size = wx.DefaultSize, style = wx.CAPTION|wx.CLOSE_BOX|wx.DEFAULT_DIALOG_STYLE )
        ...
        self.output_dir_ctrl = wx.TextCtrl(self, 
                value = os.path.join(os.path.dirname(board.GetFileName()), output_fab.defaults['output_dir'])
                )
        ...
        static_text =  wx.StaticText(self, label = "Protel Extensions")
        pars_sizer.Add(static_text, flag = label_flags, border = spacing)
        self.protel_extensions_ctrl = wx.CheckBox(self)
        self.protel_extensions_ctrl.SetValue(False)
        pars_sizer.Add(self.protel_extensions_ctrl, flag = ctrl_flags, border = spacing)
    ...
    def onRun(self, event):
        ...
        protel_ext = self.protel_extensions_ctrl.IsChecked()
